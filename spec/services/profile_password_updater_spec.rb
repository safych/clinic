...
RSpec.describe ProfilePasswordUpdater do
  ...
  it 'return sucesful update' do
    ...
    result = ProfilePasswordUpdater.call(doctor, '123456', '123456')
  ...
  it 'return error length password' do
    doctor = Doctor.find(1)
    result = ProfilePasswordUpdater.call(doctor, '1234', '1234')
    expect(result.status).to eq false
  ...
  it 'return error pwd confirmation' do
    doctor = Doctor.find(1)
    result = ProfilePasswordUpdater.call(doctor, '1234567', '123456')
    expect(result.status).to eq false
